package com.deeplake.nesretro.item.skills;

import com.deeplake.nesretro.IdlFramework;
import com.deeplake.nesretro.init.ModCreativeTab;
import com.deeplake.nesretro.item.ItemBase;
import com.deeplake.nesretro.util.CommonFunctions;
import com.deeplake.nesretro.util.IDLSkillNBT;
import com.deeplake.nesretro.util.MessageDef;
...
import static com.deeplake.nesretro.util.CommonDef.G_SKY;
import static com.deeplake.nesretro.util.CommonDef.TICK_PER_SECOND;
import static com.deeplake.nesretro.util.CommonFunctions.SendMsgToPlayer;
import static com.deeplake.nesretro.util.CommonFunctions.isShiftPressed;
import static com.deeplake.nesretro.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "nesretro.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("nesretro.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("nesretro.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("nesretro.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("nesretro.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("nesretro.skill.shared.lv_max") : "";

        return I18n.format("nesretro.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("nesretro.skill.shared.duration_desc", val);
    }
