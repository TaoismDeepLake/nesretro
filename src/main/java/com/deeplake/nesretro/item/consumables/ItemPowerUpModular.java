package com.deeplake.nesretro.item.consumables;

import com.deeplake.nesretro.entity.creatures.EntityModUnit;
import com.deeplake.nesretro.item.ItemBase;
import com.deeplake.nesretro.util.CommonFunctions;
import com.deeplake.nesretro.util.EntityUtil;
import com.deeplake.nesretro.util.NBTStrDef.IDLNBTUtil;
...
import static com.deeplake.nesretro.util.NBTStrDef.IDLNBTDef.MARK_TOTAL_COUNT;
    ...
    public boolean itemInteractionForEntity(ItemStack stack, EntityPlayer playerIn, EntityLivingBase target, EnumHand handIn) {
        if (target instanceof EntityModUnit)
        {
            ...
            if (legalTarget.is_mechanic)
            {
                ...
                {
                    CommonFunctions.SafeSendMsgToPlayer(playerIn, "nesretro.msg.upgrade_module.req_lv", String.valueOf(requireLv));
                    return false;
                }
                ...
                if (!playerIn.world.isRemote)
                {
                    ...
                    if (success)
                    {
                        ...
                        CommonFunctions.SafeSendMsgToPlayer(playerIn, "nesretro.msg.upgrade_module.ok", valueBefore, valueAfter);
