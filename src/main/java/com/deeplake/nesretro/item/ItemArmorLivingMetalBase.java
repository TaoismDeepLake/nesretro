package com.deeplake.nesretro.item;

import com.deeplake.nesretro.util.CommonFunctions;
import com.deeplake.nesretro.util.IHasModel;
...
import static com.deeplake.nesretro.util.NBTStrDef.IDLNBTDef.BIOMETAL_WARNED;
		...
		if (!worldIn.isRemote)
        {
            if (((float)stack.getItemDamage() / stack.getMaxDamage()) >= 0.9f)
            {
                if (worldIn.getWorldTime() % TICK_PER_SECOND == 0 && !IDLNBTUtil.GetBoolean(stack, BIOMETAL_WARNED, false)) {
                    ...
                    CommonFunctions.SendMsgToPlayer((EntityPlayerMP) player, "nesretro.msg.low_dura", stack.getDisplayName());
                }
            }
            else {
...
//
//        //tooltip.add(I18n.format("nesretro.gua_enhance_total.desc", IDLSkillNBT.GetGuaEnhanceTotal(stack)));
